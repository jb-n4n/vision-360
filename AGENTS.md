...
- **Daemons persistentes:** `ocr_server.py` es el **servicio unificado** (POST /ocr, /ask, /chart, /vision; GET /health; puerto canónico default 8131, el mismo de `vision360.py --daemon-url`; `--timeout 0` = servicio permanente; modelos perezosos: OCR/visión/chart se cargan en su primera petición). `chart_server.py` (POST /chart, puerto default 8080) queda como CLI legacy. Ambos: modelos cargados UNA vez, auto-cierre tras `--timeout` (default 3600 s) sin peticiones de inferencia; `/health` NO reinicia el temporizador; PaddleX no es thread-safe → un solo hilo. Autostart/gestión: `scripts/servicio-ollama.ps1` y `scripts/servicio-vision.ps1`.
