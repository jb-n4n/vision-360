...
| `scripts/experimento_captcha.py` | **Objetivo secundario (investigación):** reto visual tipo captcha resuelto en un **navegador temporal** (Playwright, contexto fresco desechable): página demo local 3×3 → RT-DETR (`/vision` objetos) + VLM SoM (`/ask` ollama) → clic en las celdas → veredicto. `--real` abre el demo de reCAPTCHA v2 (experimental). |
