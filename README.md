...
| `scripts/experimento_captcha.py` | **Objetivo secundario (investigación):** reto visual tipo captcha en un **navegador temporal** (Playwright, contexto fresco desechable). **El modo REAL es el default por decisión del programador** (abre el demo oficial de reCAPTCHA v2 y clickea el checkbox); `--local` usa una demo sintética 3×3 determinista: RT-DETR (`/vision` objetos) + VLM SoM (`/ask` ollama) → clic en las celdas → veredicto. Tras un rechazo espera el re-render de Google y reintenta hasta `--intentos` rondas (default 3). |
