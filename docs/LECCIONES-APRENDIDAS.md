...
8. **Validacion en vivo (2026-08-07) — bug de ORDEN del flag mkldnn (issue #18162):** con el daemon caliente (OCR/chart ya cargados -> paddlex importado), /vision objetos responde HTTP 500 `ConvertPirAttribute2RuntimeAttribute` aunque `modo_objetos` fije el flag justo antes del import perezoso: el flag se lee al IMPORTAR paddlex y en el daemon ya estaba importado. Fix verificado por tests: `PADDLE_PDX_ENABLE_MKLDNN_BYDEFAULT=0` al importar `ocr_server.py` y `vision.py` (setdefault, test de regresion en test_vision.py). El reintento por celda (pendiente 3) funciono EN VIVO: 2 intentos + 3 s + celda saltada, y el loop del pendiente 2 tambien (3 rondas completas; Google no re-renderizo el reto entre rondas — misma instruccion "Select all images with cars" en las 3 — y el espera re-render hizo timeout tolerante, como esta disenado). El daemon activo sigue con el bug hasta su reinicio (ver pendiente 5).
9. **Validacion en vivo tras el fix (2026-08-07) — tablas acumuladas y captura en blanco:** con el daemon reiniciado, RT-DETR detecta en vivo (buses celda 5 a 0.84; bicycles celdas 1 y 3) y el click JS marca la celda correcta. Tras un rechazo, Google APPENDEA una tabla nueva y deja la vieja en el DOM: `tiles en el DOM: 32` con la seleccion por frame -> se analizaba y clickeaba la tabla VIEJA. Fix A: tabla `.last` y tiles DENTRO de esa tabla (commit 7a103e1). Segundo bug: tras el rechazo la tabla nueva aparece con los tiles VACIOS y las imagenes cargan asincronicamente -> el screenshot salia BLANCO (verificado: 390x390, 1 color, 1321 bytes) y el detector no veia nada (rondas 2-3 vacias). Fix B (src capturado ANTES del VERIFY para el espera de re-render, commit 53fa178) y Fix C: esperar `img.complete && naturalWidth > 0` en todos los tiles antes de capturar + recaptura si < 4 KB (commit 9d225c9). Pendiente: re-validar en vivo que las rondas 2-3 resuelven con imagenes cargadas; el timeout de re-render persiste (los srcs no cambian dentro de 45 s: Google deja el reto igual o deshabilitado tras un rechazo) — investigar con captura del estado del reto tras el rechazo.
10. **Segunda validacion en vivo (2026-08-07) — dos bugs PROPIOS cazados por el run (hallazgo 11):** (a) `FrameLocator.wait_for_function` NO existe: el wait de imagenes del fix C lanzaba AttributeError silencioso (except -> "capturando igual") y la captura volvia a salir en blanco (1321 bytes) en las rondas 2-3; el wait va sobre el Frame REAL via `page.frames`. (b) La imagen del tile NO esta en el td (`backgroundImage="none"`): los tiles llevan un <img> ANIDADO con clase `rc-image-tile-33`/`-44` (URL `payload?p=...`; hallazgo 12): el selector `img.rc-image-tile` (sin sufijo) no existe y la deteccion de re-render por src no podia funcionar. Fix (commits ac292a8, a4ee614): wait en Frame real con condicion img completa O background no vacio; src del tile = `td.rc-imageselect-tile img` (clase real); captura de la cuadricula guardada POR RONDA (`captcha_reto_grid_N.png`) para analisis offline (pendiente 1). El reto "a bus" (3x3) fue rechazado con [5] detectado a 0.92: precision adversarial documentada (~50-70%).
12. **Tercera validacion en vivo (2026-08-07) — las rondas 2-3 YA RESUELVEN (fix de tabla con contenido, commit 49ca63f):** ronda 2 detecto y clickeo bus celda 6 @0.83 (antes: captura en blanco -> celdas vacias). Hallazgos del diagnostico en vivo: (a) el boton de SKIP real es `div.verify-button-holder` con texto "Skip" (el mismo del VERIFY con etiqueta cambiada); fix con click al button interno (commit 49ca63f) — pendiente 4 validado en el codigo, falta verlo en vivo. (b) la imagen del tile es `<img class="rc-image-tile-33">` dentro de `div.rc-image-tile-target > div.rc-image-tile-wrapper` (corrige hallazgo 11b: si hay imgs, pero con clase con sufijo). (c) tras 2 rechazos seguidos Google puede servir el reto vacio/blanco (ronda 3: 1321 bytes con imgs "complete" en el DOM) — probable rate-limit del demo, no bug de captura.

**Pendientes (seguir):**
1. Verificar la pasada por celda offline sobre la cuadricula 4x4 de motos guardada (captcha_reto_grid.png en temp) — el daemon se reinicio a mitad y corto la prueba. **Nota (2026-08-07):** el mapeo bbox->celda quedo cubierto por tests de regresion (`tests/test_experimento_captcha.py`): la pasada COMPLETA mapea por centro del bbox en espacio de imagen completa (correcto, 1/5/9), y la pasada POR CELDA asigna siempre al numero del loop (la posicion del bbox dentro del recorte no cambia la celda). Pendiente: re-generar la cuadricula real y verificar la pasada por celda sobre ella.
...
3. ~~Evaluar /vision por CLI (subproceso) en vez de n*n llamadas HTTP al daemon, o reintento por celda, para resistir reinicios del daemon.~~ **HECHO (2026-08-07):** se eligio la opcion del **reintento por celda** (`_llamar_con_reintentos`, 2 intentos con 3 s de espera; celda saltada solo si ambos fallan) en vez de la CLI/subproceso, que cargaria una instancia extra de RT-DETR en memoria (evitar duplicar modelos; leccion 16). Helper puro testeable: 5 tests nuevos (helper + cableado con mock de `_post`, grid PIL 60x60): 111 tests OK en el verificador. Pendiente: validacion en vivo simulando el corte del daemon a mitad del loop.
4. Validar SKIP en vivo (reto crosswalks con "If there are none, click skip"). **Avance (2026-08-07):** el reto real de buses trajo "If there are none, click skip" (skip=True parseado) pero el boton no se encontro con los selectores actuales. Se agrego `_diagnostico_botones` (imprime tag/id/clase/texto/visibilidad de botones del bframe al fallar el SKIP) y el espera de re-render ahora imprime srcs anterior/actual tras timeout (commit 8542bbd): la proxima pasada en vivo dara los selectores reales.
5. **NUEVO (2026-08-07):** reiniciar el daemon para que tome el flag `PADDLE_PDX_ENABLE_MKLDNN_BYDEFAULT=0` al importar (commit b8f4fe2) y re-validar /vision objetos en vivo; luego repetir la validacion del reto (los 3 intentos previos resolvieron con el detector caido: celdas vacias -> VERIFY sin marcar -> rechazo logico). **HEcho (2026-08-07):** daemon reiniciado por el programador; /vision objetos verificado en vivo (probe 200 OK + detecciones en el reto real). **Demo local validada CORRECTO (2026-08-07):** `--local` completo (Playwright + daemon + pasada por celda + clic + veredicto JS) -> celda 5 (person, scores 0.60-0.90) -> CORRECTO. Nota: el daemon 8131 es COMPARTIDO con otros proyectos (resultados=100, /ask de otros clientes): la pasada por celda puede encolarse detras de trabajos largos ajenos (~10 s por celda en este host + esperas). Resta solo la re-validacion de las rondas 2-3 del reto REAL con los fixes A/B/C.
