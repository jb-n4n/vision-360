...
10. **Segunda validacion en vivo (2026-08-07) — dos bugs PROPIOS cazados por el run (hallazgo 11):** (a) `FrameLocator.wait_for_function` NO existe: el wait de imagenes del fix C lanzaba AttributeError silencioso (except -> "capturando igual") y la captura volvia a salir en blanco (1321 bytes) en las rondas 2-3; el wait va sobre el Frame REAL via `page.frames`. (b) Los tiles de reCAPTCHA NO llevan `<img>`: la imagen es `background-image` del `td` (src_anterior/src_actual = None en el diagnostico): la deteccion de re-render por `img.src` no podia funcionar jamas. Fix (commit ac292a8): wait en Frame real con condicion img completa O background no vacio; "src" del tile = `getComputedStyle(td).backgroundImage`; captura de la cuadricula guardada POR RONDA (`captcha_reto_grid_N.png`) para analisis offline (pendiente 1). El reto "a bus" (3x3) fue rechazado con [5] detectado a 0.92: precision adversarial documentada (~50-70%).
